...
static void pgfdw_subxact_callback(SubXactEvent event, SubTransactionId mySubid,
...
PGconn *GetConnection(UserMapping *user, bool will_prep_stmt)
{
	...
	if (ConnectionHash == NULL) {
		...
		ConnectionHash =
			hash_create("postgres_fdw connections", 8, &ctl,
	...
	if (retry) {
		...
		ereport(DEBUG3,
			(errmsg_internal(
				...
				entry->conn)),
			errdetail_internal(
				"%s", pchomp(PQerrorMessage(entry->conn))));
...
static void make_new_connection(ConnCacheEntry *entry, UserMapping *user)
{
	...
	entry->mapping_hashvalue = GetSysCacheHashValue1(
...
static void begin_remote_xact(ConnCacheEntry *entry)
{
	...
	while (entry->xact_depth < curlevel) {
		...
		snprintf(sql, sizeof(sql), "SAVEPOINT s%d",
			 entry->xact_depth + 1);
		entry->changing_xact_state = true;
		do_sql_command(entry->conn, sql);
		entry->xact_depth++;
 ...
 * Caller is responsible for the error handling on the result.
 */
void pgfdw_get_result_dst(PGconn *conn, const char *query, PGresult **dst)
{
	...
	PG_TRY();
	{
		for (;;) {
			...
			while (PQisBusy(conn)) {
				...
				wc = WaitLatchOrSocket(
					MyLatch,
					WL_LATCH_SET | WL_SOCKET_READABLE |
						WL_EXIT_ON_PM_DEATH,
					PQsocket(conn), -1L, PG_WAIT_EXTENSION);
				ResetLatch(MyLatch);
	...
	PG_TRY();
	{
		...
		ereport(elevel,
			(errcode(sqlstate),
			 ...
			 message_context ? errcontext("%s", message_context) :
					   0,
	...
	while ((entry = (ConnCacheEntry *)hash_seq_search(&scan))) {
		...
		if (entry->xact_depth > 0) {
			...
			case XACT_EVENT_ABORT:
				...
				} else if (!pgfdw_exec_cleanup_query(
						   entry->conn,
						   "ABORT TRANSACTION",
						   false)) {
					/* Unable to abort remote transaction. */
					abort_cleanup_failure = true;
				} else if (entry->have_prep_stmt &&
					   entry->have_error &&
	...
	PG_TRY();
	{
		for (;;) {
			...
			while (PQisBusy(conn)) {
				...
				cur_timeout = TimestampDifferenceMilliseconds(
					now, endtime);
				if (cur_timeout <= 0) {
					timed_out = true;
					goto exit;
